...
class ComputeMetrics:
    metric_types = ["accuracy", "accuracy_wo_bg", "overlap_score", "IoD", "IoD_wo_bg"]
    ...
    def print_scores(self, metric_types=None):
        ...
        txt = "---Li's eval \n" + "\n".join(["*{}: {:.04}".format(metric_types[i], scores[i]) for i in range(len(metric_types))])
        print(txt)

    def get_scores(self, metric_types=None):
        if metric_types is None:
            metric_types = self.metric_types
...
def classification_accuracy(P, Y, bg_class=None, **kwargs):
    ...
    def clf_(p, y, bg_class):
        ...
        S_true = eval_utils.segment_labels(y)
        I_true = np.array(eval_utils.segment_intervals(y))
...
def edit_score(P, Y, norm=True, bg_class=None, **kwargs):
    if type(P) == list:
        ...
        P_ = eval_utils.segment_labels(P)
        Y_ = eval_utils.segment_labels(Y)
        if bg_class is not None:
...
def overlap_f1(P, Y, n_classes=0, bg_class=None, overlap=.1, **kwargs):
    def overlap_(p, y, n_classes, bg_class, overlap):
        ...
        true_intervals = np.array(eval_utils.segment_intervals(y))
        true_labels = eval_utils.segment_labels(y)
        pred_intervals = np.array(eval_utils.segment_intervals(p))
        pred_labels = eval_utils.segment_labels(p)
...
def IoD(P, Y, **kwargs):
    ...
    def overlap_d(p, y, bg_class=None):
        true_intervals = np.array(eval_utils.segment_intervals(y))
        true_labels = eval_utils.segment_labels(y)
        pred_intervals = np.array(eval_utils.segment_intervals(p))
        pred_labels = eval_utils.segment_labels(p)
        ...
        #print('pred:', pred_intervals, pred_labels)
        ...
        #print(seg_scores * 100)
        return seg_scores.mean() * 100

    if type(P) == list:
        return np.mean([overlap_d(P[i], Y[i], bg_class=None) for i in range(len(P))])
    else:
        return overlap_d(P, Y, bg_class=None)

def IoD_wo_bg(P, Y, bg_class=None, **kwargs):
    # From ICRA paper:
    # Learning Convolutional Action Primitives for Fine-grained Action Recognition
    # Colin Lea, Rene Vidal, Greg Hager
    # ICRA 2016
...
def IoU(P, Y, **kwargs):
    ...
    def overlap_d(p, y, bg_class=None):
        ...
        for i in range(n_true_segs):
            for j in range(n_pred_segs):
                if true_labels[i] == pred_labels[j]:
                    intersection = max(min(pred_intervals[j][1], true_intervals[i][1]) - max(pred_intervals[j][0],
                                                                                         true_intervals[i][0]), 0)
                    union = pred_intervals[j][1] - pred_intervals[j][0] + true_intervals[i][1] - true_intervals[i][0] - intersection
                    score_ = float(intersection) / union
                    seg_scores[i] = max(seg_scores[i], score_)

        #print('true:', true_intervals, true_labels)
        #print('pred:', pred_intervals, pred_labels)
        #print('score:', seg_scores)
        acc = (np.array(y) == np.array(p)).mean()
        #print(len(true_labels), len(pred_labels), true_intervals[-1][-1], seg_scores.mean(), acc)
        res = {'true_intervals': true_intervals.tolist(), 'true_labels': true_labels.tolist(), \
                'pred_intervals': pred_intervals.tolist(), 'pred_labels': pred_labels.tolist(),\
                'seg_scores': seg_scores.tolist(), 'acc': acc}
        #print(json.dumps(res))
        res_list.append(res)
        return seg_scores.mean() * 100

    if type(P) == list:
        val = np.mean([overlap_d(P[i], Y[i], bg_class=None) for i in range(len(P))])
        return val
    else:
        val = overlap_d(P, Y, bg_class=None)
        return val

def IoU_wo_bg(P, Y, bg_class=None, **kwargs):
    # From ICRA paper:
    # Learning Convolutional Action Primitives for Fine-grained Action Recognition
    # Colin Lea, Rene Vidal, Greg Hager
    # ICRA 2016
